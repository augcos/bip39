...
var invalidChecksumMnemonic = "exile ask congress lamp submit jacket era scheme attend cousin alcohol catch course end lucky hurt sentence oven short ball bird grab wing race"
var invalidChecksumEntropy = "77c2b00716cec7213839159e404db50da7"


/************************************************** Test Functions ****************************************************/

// TestValidBitSize tests multiple bit sizes and checks if the returned error for each size is appropiate
func TestValidBitSize(t *testing.T) {
	for i:=0; i<=256; i++ {
		_, err := GenEntropy(i)

		if (i%32==0) && (i>=128) && (i<=256) {
			if err!=nil {
				t.Errorf("Bit size %v should be valid", i)
			}
		} else {
			if err==nil {
				t.Errorf("Bit size %v should be invalid", i)
			}
		}
	}
}
...
// TestValidMnemonic tests for error detection in seed generation from invalid mnemonics
func TestValidMnemonic(t *testing.T) {
	for _, vector := range vectorSlice {
		testMnemonic := strings.Split(vector.mnemonic, " ")
		testMnemonic = testMnemonic[:len(testMnemonic)-1]
		_, err := GetSeedFromMnemonic(testMnemonic, "TREZOR")
		if err==nil {
			t.Errorf("Corrupt mnemonic should not be valid")
		}
	}
}


// TestGetMnemonicFromEntropy tests mnemonic generation from the test entropies
func TestGetMnemonicFromEntropy(t *testing.T) {
	for _, vector := range vectorSlice {
		testEntropy, _ := hex.DecodeString(vector.entropy)
		testEntropy, _ = AddChecksum(testEntropy)
		mnemonic, err := GetMnemonicFromEntropy(testEntropy)
		if err!=nil {
			t.Errorf("Checksum not valid (impossible to generate mnemonic from entropy)")
		}

		mnemonicJoined := strings.Join(mnemonic, " ")
		if mnemonicJoined!=vector.mnemonic {
			t.Errorf("Generated mnemonic is invalid")
		}
	}
}


// TestSeedFromMnemonic tests seed generation from the test mnemonics
func TestSeedFromMnemonic(t *testing.T) {
	for _, vector := range vectorSlice {
		testSeed, _ := hex.DecodeString(vector.seed)
		seed, err := GetSeedFromMnemonic(strings.Split(vector.mnemonic, " "), "TREZOR")
		if err!=nil {
			t.Errorf("Mnemonic is invalid: unable to generate seed")
		}

		if equal:=bytes.Compare(testSeed,seed); equal!=0 {
			t.Errorf("Generated seed is invalid")
		}
	}
}


// TestChecksumMnemonic tests for invalid checksum detection in a mnemonic
func TestChecksumMnemonic(t *testing.T) {
	_, err := GetEntropyFromMnemonic(strings.Split(invalidChecksumMnemonic, " "))
	if err==nil {
		t.Errorf("Mnemonic should be invalid: wrong checksum")
	}
}


// TestChecksumEntropy tests for invalid checksum detection in an entropy byte slice
func TestChecksumEntropy(t *testing.T) {
	testEntropy, _ := hex.DecodeString(invalidChecksumEntropy)
	_, err := GetMnemonicFromEntropy(testEntropy)
	if err==nil {
		t.Errorf("Entropy should be invalid: wrong checksum")
	}
}